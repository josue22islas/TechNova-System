 ...
 * ----------------------
 * Contiene dos dominios visuales: Distribución de costos (doughnut) y Resumen
 * financiero (líneas). Chart.js dibuja los datos; GSAP coordina el revelado.
 *
 * Entradas: valores fallback del archivo, eventos de periods.js y funciones
 * públicas updateCostChart()/updateFinancialChart() para datos del backend.
 * Salidas: canvas, leyendas, totales, tooltips HTML y eventos de presentación.
 * Regla: números y etiquetas entran sin formato; aquí se aplica es-MX y MXN.
 */
document.addEventListener('DOMContentLoaded', function () {
    ...
    const costTooltip = document.getElementById('costChartTooltip');
    const costTooltipLabel = document.getElementById('costChartTooltipLabel');
    const costTooltipValue = document.getElementById('costChartTooltipValue');
    ...
    const canAnimateEntrance =
        typeof gsap !== 'undefined'
        &&
        !reducedMotion;

    let costRevealPlayed = false;
    ...
    function playCostChartReveal() {

        if (costRevealPlayed) return;

        costRevealPlayed = true;
    ...
    function hideCostTooltip() {
        if (!costTooltip) return;

        costTooltip.classList.remove('visible');
        costTooltip.setAttribute('aria-hidden', 'true');
    }

    function externalCostTooltip(context) {
        ...
        const color = costTooltipGlowColors[index] || '#6f9fff';

        if (costTooltipLabel) {
            costTooltipLabel.textContent = labels[index] || 'Costo';
        }

        if (costTooltipValue) {
            costTooltipValue.textContent = `${money(values[index])} MXN`;
        }
        ...
        const left = Math.min(
            chartWidth - (tooltipWidth / 2) - 3,
            Math.max((tooltipWidth / 2) + 3, tooltip.caretX)
        );
        const top = Math.min(
            chartHeight - (tooltipHeight / 2) - 3,
            Math.max((tooltipHeight / 2) + 3, tooltip.caretY)
        );

        costTooltip.style.setProperty('--cost-tooltip-color', color);
        costTooltip.style.left = `${left}px`;
        costTooltip.style.top = `${top}px`;
        costTooltip.classList.add('visible');
        costTooltip.setAttribute('aria-hidden', 'false');
    }
    ...
    canvas.addEventListener('mouseleave', hideCostTooltip);
...
/* =========================================
   ...
   Si la secuencia de intro fallara,
