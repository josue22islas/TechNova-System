...
(() => {
  ...
  const notifyParent = (type) => {
    if (embedded) window.parent.postMessage({ type }, location.origin === "null" ? "*" : location.origin);
  };
  ...
  const finish = () => {
    ...
    startAmbient();
    syncControls();
    notifyParent("technova:intro-complete");
  };
  ...
  if (embedded) {
    ...
    notifyParent("technova:intro-ready");
  }
  ...
  $("[data-fullscreen]").hidden = embedded || !document.fullscreenEnabled;
